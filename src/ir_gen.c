...
/*ir_gen_result *translate(ir_gen *ir_gen,
  ...
  case function_n:
	...
	if (tree->children[2]->n.type == parameter_list_n) {
	  ...
	  for (int i = 0; i < tree->children[2]->used; i++) {
		three_address_code_add(i_result->code, MAXIMUM_LABEL, mem_copy_op,
							   address_memory, -offset, address_undefined,
							   typetable_get_size(parser_result.type_table, 1),
							   address_memory, offset);
	  }
	}
  ...
  case expression_n:
	if (tree->used == 3) {
	  long place1 = new_var();
	  long place2 = new_var();
	  translate(i_result, parser_result, place1);
	  translate(i_result, parser_result, place2);
	  three_address_code_add(i_result->code, MAXIMUM_LABEL,
							 get_op(tree->children[1]->n.type), place, place1,
							 place2);
	} else if (tree->used == 1) {
  ...
  case factor_n:
	if (tree->children[0]->n.type == var_n) {
	  ...
	  translate(i_result, tree, table, place1);
	  three_address_code_add(i_result->code, MAXIMUM_LABEL,
							 get_op(tree->children[1]->n.type), place,
							 place1, );
	} else if (tree->used == 1) {
	  long place1 = new_var();
	  translate(i_result, tree, table, place1);
